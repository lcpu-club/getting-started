...
不加约束的情况下，Windows 的 C 盘很像我们家里的衣帽间或者乡下的小仓库，什么都往里扔。最终，往往就会堆积一大堆食之无味、弃之可惜~~可能并不可惜~~的文件，这些文件我们疑似很久很久都用不上，那么不如清理了为好。以下是一些很常见的鸡肋文件：

- **系统更新残留**堆积在`C:\Windows\SoftwareDistribution\Download`
- **休眠文件**`hiberfil.sys`
- **虚拟内存**`pagefile.sys`
- **临时文件**散落在`~\AppData\Local\Temp`
...
以下内容经常涉及到**管理员权限**。你可能需要启用账户 Administrator。
