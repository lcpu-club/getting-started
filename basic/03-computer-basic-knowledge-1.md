...
图3. 一根台式机内存条，来自尔必达(日本)，后为镁光公司收购。由于单颗内存芯片性能和性价比受限，故而将若干颗内存芯片并列组装成一个条形的内存模块。  
...
图4. 机械硬盘内部结构。图源自wiki，获CC BY-SA授权
![SDD](/assets/basic/03-computer-basic-knowledge-1/SSD.jpg)
图5. 固态硬盘内部结构。左SandForce标志芯片为主控，负责与主机通讯以及管理和读写右侧6颗闪存颗粒内的数据。图源自wiki，获CC BY-SA授权
...
图7. 一台笔记本电脑内部，上部蓝色主板上集合了若干各式功能的组件 (笔者自摄)
