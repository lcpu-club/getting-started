...
- RiscV: 新兴的开源ISA，嵌入式设备中有使用，由于其设计比较简单，学校也喜欢用于教学
- Loongarch: 国产ISA
...
> 历史的注记：Intel当年受到HP的“蛊惑”联合开发了一个全新的64位架构IA64，处理器代号为安腾(Itanium)。该架构具有很多让人眼前一亮的特性，然而与之前Intel的x86完全不兼容。而AMD选择开发了与之前x86架构兼容的AMD64架构，迅速抢占市场份额。由于安腾落后的性能和开发速度，市场表现不佳，因此转而采用AMD64架构，或者更中立的名称，x86-64架构，而安腾逐渐被历史淘汰。
...
GPU用于图形处理加速和视频输出(因此如果使用独立显卡输出，显示器视频线需要连到显卡的接口，而不是主板上的)，以及包含了一些视频编解码电路。(省流:可以用来玩原神)  
...
线程(Thread)是轻量级的进程。一般进程具有处理器和内存地址的隔离，而一个进程下的线程仅有处理器的隔离，虚拟内存地址是共用的。这样不同线程之间的通信成本要比进程之间小很多。

##### 例外

例外(Exception)，有些时候翻译为异常(此处异常通常指代abnormality，对于硬件系统中通常异常不等于错误)，通常后者称呼使用更多(后续使用异常这一称呼)，即区别于程序正常进行的例外的情况。
